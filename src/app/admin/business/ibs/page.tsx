...
const initialCategories: Category[] = [
  { id: 1, nameKo: "통합배선공사", nameEn: "Integrated Wiring" },
  { id: 2, nameKo: "CCTV / CATV", nameEn: "CCTV / CATV" },
  { id: 3, nameKo: "AV 시스템", nameEn: "AV System" },
  { id: 4, nameKo: "서버실 구축", nameEn: "Server Room" },
];

const initialItems: IBSItem[] = [
  { id: 1, titleKo: "카테고리6 UTP 케이블 인프라 구축", titleEn: "Cat.6 UTP Cable Infrastructure", categoryId: 1, image: "" },
  { id: 2, titleKo: "광케이블 백본 네트워크", titleEn: "Fiber Optic Backbone Network", categoryId: 1, image: "" },
  { id: 3, titleKo: "IP 카메라 CCTV 시스템", titleEn: "IP Camera CCTV System", categoryId: 2, image: "" },
];

export default function AdminIBSPage() {
  const [categories, setCategories] = useState<Category[]>(initialCategories);
  const [items, setItems] = useState<IBSItem[]>(initialItems);
  ...
  const removeCategory = (id: number) => {
    if (!confirm("이 카테고리와 관련 항목을 삭제하시겠습니까?")) return;
  ...
  const updateItem = (id: number, field: keyof IBSItem, value: string | number) => {
  ...
  const handleSave = () => {
    console.log("IBS 저장:", { categories, items });
    setSaved(true);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-2xl font-bold text-gray-900">IBS 통합시스템 관리</h1>
        <button
          onClick={handleSave}
        ...
        <div className="space-y-2 mb-3">
          {categories.map((cat) => (
            <div key={cat.id} className="flex items-center gap-2 bg-gray-50 rounded-lg p-2">
              <input type="text" value={cat.nameKo} onChange={(e) => updateCategoryName(cat.id, "nameKo", e.target.value)} placeholder="카테고리명 (KO)" className="flex-1 px-3 py-1.5 bg-white border border-gray-200 rounded text-sm text-gray-700 outline-none focus:ring-2 focus:ring-blue-500" />
          ...
          <div key={item.id} className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-4">
            ...
            <div className="flex-1 space-y-2">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <input type="text" value={item.titleKo} onChange={(e) => updateItem(item.id, "titleKo", e.target.value)} placeholder="제목 (KO)" className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm text-gray-900 outline-none focus:ring-2 focus:ring-blue-500" />
                <input type="text" value={item.titleEn} onChange={(e) => updateItem(item.id, "titleEn", e.target.value)} placeholder="Title (EN)" className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm text-gray-900 outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
