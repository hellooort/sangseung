...
const initialItems: IBSItem[] = [
  { id: 1, title: "카테고리6 UTP 케이블 인프라 구축", categoryId: 1, image: "" },
  { id: 2, title: "광케이블 백본 네트워크", categoryId: 1, image: "" },
  { id: 3, title: "IP 카메라 CCTV 시스템", categoryId: 2, image: "" },
];
    ...
    <div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
          ...
          {categories.map((cat) => (
            <div key={cat.id} className="flex items-center gap-1 bg-gray-100 rounded-lg px-1">
              <input
                ...
                className="px-2 py-1.5 bg-transparent text-sm text-gray-700 outline-none w-28"
              />
        ...
        <div className="flex gap-2">
          ...
          />
          <button onClick={addCategory} className="bg-gray-800 text-white px-4 py-2 rounded-lg text-sm hover:bg-gray-900">
            추가
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium ${activeCategory === null ? "bg-blue-600 text-white" : "bg-white text-gray-600 border border-gray-200"}`}
        >
          전체
        </button>
        {categories.map((cat) => (
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium ${activeCategory === cat.id ? "bg-blue-600 text-white" : "bg-white text-gray-600 border border-gray-200"}`}
          >
            {cat.name}
          </button>
        ))}
